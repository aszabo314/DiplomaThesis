...
\\
Shape detection is performed one small regions only. Section \ref{sec:user_guided_sd} describes an interactive heuristic that selects an octree node as region of interest, using the user's cursor position and camera view as input. This effectively changes the task of detecting shapes from an automated approach to an user-controlled interaction.
\\
